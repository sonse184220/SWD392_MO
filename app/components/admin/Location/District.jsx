...
import { getDistrictList, addDistrict, deleteDistrict, updateDistrict } from "../../../Service/districtService";
...
const District = ({ theme }) => {
    ...
    const handleSave = async () => {
        ...
        try {
            setIsLoading(true);
            if (currentCity) {
                await updateDistrict(currentCity.cityId, { name, description });
            } else {
                await addDistrict({ name, description });
            }
            fetchDistricts();
            setIsOpen(false);
        } catch (error) {
    ...
    const handleDelete = async () => {
        ...
        try {
            ...
            await deleteDistrict(districtToDelete.districtId);
            fetchDistricts();
        } catch (error) {
    ...
    const renderEmptyState = () => (
        <View style={{ flex: 1, alignItems: 'center', justifyContent: 'center', paddingVertical: 40 }}>
            ...
            <Text style={{ fontSize: 18, color: theme.colors.elevation.level5, marginTop: 16 }}>No districts found</Text>
            <Text style={{
                fontSize: 14,
                ...
                Add your first district by clicking the "Add District" button above
            </Text>
        </View>
    );
    ...
    return (
        <View style={{ flex: 1, padding: 20, backgroundColor: theme.colors.background }}>
            ...
            <View style={{ marginBottom: 24 }}>
                <Text style={{ color: theme.colors.elevation.level5, marginTop: 4 }}>Add, edit and manage district information</Text>
            </View>
            ...
            {isLoading && districts.length === 0 ? (
                <View style={{ flex: 1, alignItems: 'center', justifyContent: 'center' }}>
                    ...
                    <Text style={{ marginTop: 16, color: theme.colors.elevation.level5 }}>Loading districts...</Text>
                </View>
            ) : (
                <FlatList
                    data={districts}
                        ...
                        >
                            <Card.Content>
                                ...
                                <View style={{
                                    ...
                                    <Text style={{ fontSize: 12, color: theme.colors.elevation.level5 }}>City ID: {item.cityId}</Text>
                                </View>
                            </Card.Content>
                            <Card.Actions style={{
                                flexDirection: 'row',
                    ...
                    contentContainerStyle={districts.length === 0 ? { flex: 1 } : {}}
                />
            )}
            ...
            >
                <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 24 }}>
                    ...
                    <Text style={{ fontSize: 20, fontWeight: 'bold', color: theme.colors.onSurface }}>
                        {currentDistrict ? "Edit District" : "Add New District"}
                    </Text>
                </View>

                <View style={{ marginBottom: 16 }}>
                    <Text style={{ fontSize: 14, fontWeight: '500', marginBottom: 8, color: theme.colors.onSurface }}>District Name *</Text>
                    <TextInput
                ...
                <View style={{ marginBottom: 24 }}>
                    <Text style={{ fontSize: 14, fontWeight: '500', marginBottom: 8, color: theme.colors.onSurface }}>City ID *</Text>
                    <TextInput
